                                ...
                                bool normalsEnabled, sf::Shader& normalsShader)
{
    ...
    std::vector<LightEngine::Penumbra> penumbras;
    ...
    for (unsigned i = 0; i < shapesCount; ++i) {
        ...
        penumbras.clear();
        LightEngine::getPenumbrasPoint(penumbras, innerBoundaryIndices, innerBoundaryVectors, outerEdges[i]._outerBoundaryIndices, outerEdges[i]._outerBoundaryVectors, pLightShape->_shape, castCenter, _sourceRadius);
